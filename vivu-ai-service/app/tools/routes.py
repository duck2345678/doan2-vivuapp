...
def estimate_duration_minutes(distance_km: float) -> int:
    if distance_km <= 0:
        return 0
    return max(MIN_TRAVEL_MINUTES, int(math.ceil(distance_km / AVERAGE_URBAN_SPEED_KMH * 60)))


def _haversine_segment(origin: PlaceCandidate, destination: PlaceCandidate) -> RouteSegment:
    distance = round(haversine_km(origin.latitude, origin.longitude, destination.latitude, destination.longitude), 2)
    return RouteSegment(
        from_place_id=origin.place_id,
        to_place_id=destination.place_id,
        distance_km=distance,
        duration_minutes=estimate_duration_minutes(distance),
        provenance=Provenance(source="INTERNAL_ESTIMATE", is_estimate=True, retrieved_at=utc_now_iso()),
    )
...
class RouteProvider(ABC):
    ...
    def calculate_route_segments(self, places: Sequence[PlaceCandidate]) -> List[RouteSegment]:
        return [self.calculate_route(places[i], places[i + 1]) for i in range(len(places) - 1)]
...
class GoogleRoutesProvider(RouteProvider):
    ...
    def __init__(self, fallback_provider: Optional[RouteProvider] = None, http_client: Optional[httpx.Client] = None):
    ...
    def calculate_route(self, origin: PlaceCandidate, destination: PlaceCandidate) -> RouteSegment:
        ...
        for attempt in range(2):
            try:
                ...
                last_error = exc
                logger.warning("Google Routes timeout/transport, retry=%s", attempt + 1)
    ...
    def _request_google_route(self, origin: PlaceCandidate, destination: PlaceCandidate) -> RouteSegment:
        headers = {
        ...
        if self._http_client:
            response = self._http_client.post(self.ROUTES_URL, headers=headers, json=body, timeout=timeout)
        else:
            with httpx.Client(timeout=timeout) as client:
                response = client.post(self.ROUTES_URL, headers=headers, json=body)
        ...
        seconds = int(float(duration_raw.rstrip("s"))) if duration_raw.endswith("s") else int(float(duration_raw))
        return RouteSegment(
            from_place_id=origin.place_id,
            to_place_id=destination.place_id,
            distance_km=round(meters / 1000.0, 2),
            duration_minutes=max(0, int(math.ceil(seconds / 60))) if seconds else estimate_duration_minutes(meters / 1000.0),
...
def calculate_route(origin: PlaceCandidate, destination: PlaceCandidate, provider: Optional[RouteProvider] = None) -> RouteSegment:
    return (provider or get_default_routes_provider()).calculate_route(origin, destination)


def calculate_route_segments(places: Sequence[PlaceCandidate], provider: Optional[RouteProvider] = None) -> List[RouteSegment]:
